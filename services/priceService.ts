...
// Используем ключ напрямую, раз переменные окружения подводят
...
export const fetchRealtimePriceData = async (timeframe: Timeframe): Promise<{ data: PriceData[], sources: GroundingSource[], isFallback: boolean }> => {
  try {
    ...
    const prompt = `SEARCH ONLINE for actual FCPO (Crude Palm Oil) futures prices on Bursa Malaysia. Today is ${new Date().toDateString()}. Provide daily OHLC data for ${timeframe}. Return ONLY JSON: {"prices": [{"date": "ISO", "open": number, "high": number, "low": number, "close": number}]}.`;
